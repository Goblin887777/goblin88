...
function get_firebase_data_final($path) {
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 5); // Biar gak kelamaan nunggu kalau firebase lemot
    $res = curl_exec($ch);
    curl_close($ch);
...
// 1. AMBIL SALDO
$user_data = get_firebase_data_final('users_situs1/' . $username);
$saldo_raw = (float)($user_data['saldo'] ?? 0); 
$saldo_game = number_format($saldo_raw, 2, '.', ','); 

// 2. AMBIL DATA
$userHistory = get_firebase_data_final('history/' . strtolower($username));
$pendingDepo = get_firebase_data_final('deposit_pending_situs1');
$pendingWD   = get_firebase_data_final('withdraw_requests_situs1'); 

$history = [];
$totalCredit = 0;
$totalDebit = 0;

// --- A. PROSES HISTORY (Sangat Rawan Error kalau gak di-check is_array) ---
if (!empty($userHistory) && is_array($userHistory)) {
    foreach ($userHistory as $id => $d) {
        if (!is_array($d)) continue; // KUNCI BIAR GAK ERROR 505/500

        $d['trans_id'] = $id;
        $d['waktu'] = $d['waktu'] ?? $d['created_at'] ?? '-';
        $history[] = $d;

        $status = strtolower($d['status'] ?? '');
        $amt = (float)($d['nominal'] ?? $d['amount'] ?? 0);
        $type = strtolower($d['type'] ?? $d['tipe'] ?? '');

        if ($status == 'success' || $status == 'sucses' || $status == 'berhasil') {
            if (strpos($type, 'depo') !== false || strpos($type, 'qris') !== false) {
                $totalCredit += $amt;
            } elseif (strpos($type, 'with') !== false || strpos($type, 'wd') !== false) {
                $totalDebit += $amt;
            }
        }
    }
...
// --- B. PROSES DEPOSIT PENDING ---
if (!empty($pendingDepo) && is_array($pendingDepo)) {
    foreach ($pendingDepo as $id => $p) {
        if (!is_array($p)) continue; 
        
        $uCheck = strtolower($p['username'] ?? $p['user_id'] ?? '');
        if ($uCheck === strtolower($username)) {
            $p['trans_id'] = $id;
            $p['status'] = 'Pending';
            $p['type'] = $p['tipe'] ?? 'Deposit';
            $p['waktu'] = $p['waktu'] ?? $p['created_at'] ?? '-';
            $history[] = $p;
        }
    }
}

// --- C. PROSES WITHDRAW PENDING ---
if (!empty($pendingWD) && is_array($pendingWD)) {
    foreach ($pendingWD as $id => $w) {
        if (!is_array($w)) continue;

        $uCheck = strtolower($w['username'] ?? $w['user_id'] ?? '');
        if ($uCheck === strtolower($username)) {
            $w['trans_id'] = $id;
            $w['status'] = 'Pending';
            $w['type'] = 'Withdraw';
            $w['nominal'] = $w['amount'] ?? $w['nominal'] ?? 0;
            $w['waktu'] = $w['created_at'] ?? $w['waktu'] ?? '-';
            $history[] = $w;
        }
    }
}
    ...
    <li class="nav-item">
      ...
      <div class="nav-item-content" [ngclass]="{'open':isOpenNavContent[0], '':!isOpenNavContent[0]}">
        <ul class="submenu account">
          <li>
            <a href="https://www.Goblin88.site/account/deposit" (click)="closeNav($event);">
              ...
              <div class="mdc-tab-scroller__scroll-area mdc-tab-scroller__scroll-area--scroll mdc-tab-scroller__scroll-x" style="margin-bottom: 0px;">
                <div class="mdc-tab-scroller__scroll-content">
                  ...
                  <!---->
                    ...
                    <li>
                      <a role="tab" href="history.php" data-active="accountlastdirecttransfer" class="mdc-tab mdc-tab--active" aria-selected="false" tabindex="-1" id="goog_2098347606-FIXED-3">
                        <span class="mdc-tab__content">
...
<div class="row history">
    <div class="pb-4 pb-md-0 col-md-8">
        ...
        <?php 
            ...
            if (!empty($history) && is_array($history)) {
                foreach ($history as $h) {
                    ...
                    // HANYA HITUNG TOTAL JIKA STATUS SUKSES/BERHASIL
                    if (in_array($st, ['success', 'sucses', 'berhasil', 'approved', 'confirmed'])) {
                        $nom = (float)($h['nominal'] ?? $h['amount'] ?? 0);
                        $type = strtolower($h['type'] ?? $h['tipe'] ?? '');

                        // --- TAMBAHAN: Masukkan 'bonus' ke dalam perhitungan Total Credit ---
                        if (strpos($type, 'depo') !== false || strpos($type, 'credit') !== false || strpos($type, 'qris') !== false || strpos($type, 'bonus') !== false) {
                            $tampilanCredit += $nom;
                        } else {
                            $tampilanDebit += $nom;
                        }
        ...
        ?>
         ...
         <div class="history-table" style="background-color: #000000; padding: 15px;">
            <div class="row total-data text-center" style="background: linear-gradient(135deg, #e6b000 0%, #e6c400 100%) !important; padding: 15px 0; margin-bottom: 20px; border: 1px solid #e6c400 !important; border-radius: 4px; box-shadow: 0 0 12px rgba(0, 179, 255, 0.4);">
                <div class="col-xs-6 noSidePadding">
                    <div class="text-uppercase" style="font-size: 11px; color: #F0F8FF !important;">Total Credit</div>
                    <div style="font-weight: bold; color: #F0F8FF !important; font-size: 14px; text-shadow: 0 1px 2px rgba(5, 11, 20, 0.8);"> IDR <?php echo number_format($tampilanCredit, 0, ',', '.'); ?> </div>
                </div>
                <div class="col-xs-6 noSidePadding" style="border-left: 1px solid rgba(240,248,275,0.3) !important;">
                    <div class="text-uppercase" style="font-size: 11px; color: #F0F8FF !important;">Total Debit</div>
                    <div style="font-weight: bold; color: #F0F8FF !important; font-size: 14px; text-shadow: 0 1px 2px rgba(5, 11, 20, 0.8);"> IDR <?php echo number_format($tampilanDebit, 0, ',', '.'); ?> </div>
                </div>
            </div>

            <div class="history-list">
                <?php if (empty($history)): ?>
                    <div class="text-center p-5" style="color: #999 !important; background: #000;">
                        <p>Tidak ada riwayat ditemukan.</p>
                    </div>
                <?php else: ?>
                    <?php $i = 0; foreach ($history as $item): 
                        if (!is_array($item)) continue; // Anti Error 505
                        $i++; 
                    ?>
                        <?php 
                            $st = strtolower($item['status'] ?? 'pending');
                            $typeStr = strtolower($item['type'] ?? $item['tipe'] ?? '');
                            $isDepo = (strpos($typeStr, 'depo') !== false || strpos($typeStr, 'credit') !== false || strpos($typeStr, 'qris') !== false);
                            
                            // --- TAMBAHAN: Deteksi jika tipe transaksinya adalah bonus ---
                            $isBonus = (strpos($typeStr, 'bonus') !== false);
                            
                            $waktuReal = $item['waktu'] ?? $item['created_at'] ?? '-';
                            $nominalReal = (float)($item['nominal'] ?? $item['amount'] ?? 0);

                            // Background JIGJAG sesuai gambarmu (Biru dan Hitam)
                            $cardBg = ($i % 2 != 0) ? '#e6c400' : '#0a0a0a'; 

                            // Logika untuk warna Badge Status (SUDAH DIPERBAIKI)
                            if (in_array($st, ['success', 'sucses', 'berhasil', 'approved', 'confirmed'])) {
                                $displayStatus = 'Confirmed';
                                $statusBg = '#00b050'; // Hijau
                            } elseif (in_array($st, ['reject', 'rejected', 'cancel', 'failed', 'tolak', 'ditolak'])) {
                                $displayStatus = ucfirst($st);
                                $statusBg = '#dc3545'; // Merah
                            } else {
                                $displayStatus = ucfirst($st);
                                $statusBg = '#ffc107'; // Kuning (Untuk Pending)
                            }

                            // --- TAMBAHAN: Ubah metodeText jika bonus menjadi PROMO REWARD ---
                            $metodeText = ($isBonus) ? 'PROMO REWARD' : (($isDepo) ? ($item['tipe'] ?? $item['metode'] ?? 'BANK / QRIS') : 'TRANSFER BANK');
                            
                            // Menghilangkan border bottom jika background hitam
                            $borderStyle = ($cardBg == '#0a0a0a') ? '1px solid #1a1a1a' : 'none';
                        ?>

                        <!-- Mulai Kartu -->
                        <div class="row no-gutters h-result" style="background: <?php echo $cardBg; ?> !important; padding: 14px 16px; margin-bottom: 12px; border: <?php echo $borderStyle; ?> !important;">
                            <div class="col-xs-7">
                                <div style="font-weight: bold; font-size: 13px; color: #fff !important; margin-bottom: 4px;">
                                    <?php echo $waktuReal; ?>
                                </div>
                                <div style="font-size: 15px; font-weight: normal; color: #fff !important;">
                                    <!-- --- TAMBAHAN: Tampilkan keterangan bonus atau default Reward --- -->
                                    <?php echo $isBonus ? ($item['keterangan'] ?? 'Bonus Reward') : ($isDepo ? 'Deposit' : 'Withdraw'); ?>
                                </div>
                                <div style="font-size: 11px; color: #fff !important; margin-top: 5px; opacity: 0.9;">
                                    TransID:
                                </div>
                                <div style="font-size: 11px; color: #fff !important; word-break: break-all;">
                                    <?php echo $item['trans_id'] ?? $item['id'] ?? '-'; ?>
                                </div>
                            </div>

                            <div class="col-xs-5 text-right">
                                <div style="margin-bottom: 6px;">
                                    <span style="background: <?php echo $statusBg; ?> !important; color: #fff !important; padding: 2px 8px; border-radius: 2px; font-size: 11px; font-weight: bold;">
                                        <?php echo $displayStatus; ?>
                                    </span>
                                </div>
                                <div style="font-weight: bold; font-size: 15px; color: #fff !important; margin-bottom: 4px;">
                                    IDR <?php echo number_format($nominalReal, 2, '.', ','); ?> <!-- Format uang .00 -->
                                </div>
                                <div style="font-size: 12px; color: #fff !important; font-weight: bold;">
                                    <?php echo strtoupper($metodeText); ?>
                                </div>
                            </div>
                        </div>
                        <!-- Akhir Kartu -->
...
$(document).ready(function () {
    ...
    // Simulasi Modal Detail (Opsional)
    $('.h-result').click(function(){
        console.log("Detail transaksi dibuka");
    });
});
</script>
    ...
    <nav class="navbar-inverse navbar-fixed-bottom">
        <div class=" ">
                ...
                <div class="  footericon-single">
                    <a href="https://www.Goblin88.site/home.php"><i class="icon-home"></i><div>HOME</div></a>
                </div>
