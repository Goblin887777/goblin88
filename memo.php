...
<style>
    :root {
        --primary: #f1d900; /* Warna Kuning Emas */
        --bg: #121212;
        --card: #1e1e1e;
        --text: #b0b0b0;
    }
    ...
    .memo-tabs { display: flex; gap: 10px; }
    ...
    .m-tab.active {
        background: rgba(241, 217, 0, 0.15);
    ...
    .memo-item.unread { border-left-color: var(--primary); background: #252525; }
    .memo-item:hover { background: #2a2a2a; transform: translateX(5px); }

    .memo-top { display: flex; justify-content: space-between; font-size: 11px; color: var(--text); margin-bottom: 5px; }
    .memo-title { font-size: 15px; font-weight: 600; display: block; }
...
<div class="memo-container">
    <div class="memo-header">
        <div class="memo-tabs">
            <div class="m-tab active" onclick="switchTab('inbox', this)">Kotak Masuk</div>
            <div class="m-tab" onclick="switchTab('announcement', this)">Pengumuman</div>
        </div>
        <button class="btn-del" onclick="deleteChecked()">Hapus Pesan</button>
    </div>

    <div id="memo-list">
        <div style="text-align:center; padding:20px; color:var(--text)">Memuat data...</div>
    </div>
</div>
...
<script>
    ...
    async function loadMemos() {
        ...
        try {
            // Memanggil api_memo.php yang sudah kita buat sebelumnya
            const response = await fetch(`api_memo.php?mode=${mode}`);
            const data = await response.json();

            container.innerHTML = ''; 

            if (data.length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:40px; color:#666;">Tidak ada pesan.</div>';
                return;
            }

            data.forEach(memo => {
                const isUnread = memo.is_read == 0 ? 'unread' : '';
                const badge = memo.is_read == 0 ? '<span class="badge-new">BARU</span>' : '';
                
                const html = `
                    <div class="memo-item ${isUnread}" data-id="${memo.id}" onclick="openMemo(this, ${memo.id})">
                        <div class="memo-top">
                            <span>DARI: ${memo.sender.toUpperCase()}</span>
                            <span>${memo.date}</span>
                        </div>
                        <span class="memo-title">${memo.title} ${badge}</span>
            ...
            container.innerHTML = '<div style="text-align:center; padding:20px; color:red;">Tidak Ada...</div>';
        }
    }

    // 2. FUNGSI BUKA & UPDATE STATUS BACA
    function openMemo(el, id) {
        ...
        if(el.classList.contains('unread')) {
            ...
            // Mengirim sinyal ke api_memo.php agar is_read jadi 1 di database
            fetch(`api_memo.php?update_read=${id}`);
        }
    }
    ...
    function switchTab(type, el) {
        ...
        document.getElementById('mode').value = (type === 'inbox') ? 1 : 3;
        loadMemos();
    }
    ...
    // 4. FUNGSI HAPUS (Gunakan Logika SweetAlert jika ada)
    function deleteChecked() {
        if(confirm("Apakah Anda yakin ingin menghapus semua pesan di kategori ini?")) {
            const mode = document.getElementById('mode').value;
            // Kirim perintah hapus ke server
            fetch(`api_memo.php?delete_all=${mode}`)
            .then(() => loadMemos());
        }
    }
</script>
  ...
  <div class="nifty-modal slide-in-bottom downloadapk-modal" id="apk-modal">
      ...
      <h3 id="downloadgame-title" ></h3>
           ...
           <!--region Transfer Wallet Menu -->
            <div class="row no-gutters" id="trans_to_game_menu__game-modal"  >
                <form  action="" method="post" id="tw_transfer_form" class="tw_transfer_form">
                  <input type="hidden" name="_token" value="WgPVEpnhv5plHOFVkRxCRVjSKg9dox6mlRTJ4Dqu">                    <div class="form-group">
          ...
          <!--endregion Transfer Wallet Menu -->
                	...
                	<img class="img-fluid" src="https://semangatMERCUSUAR77.site/assets/images/log_html5.png" alt="play-in-browser">
  								<div class="download-caption text-center">
  										Play now in your browser
  								</div>
  								<div class="download-linkbtn text-center">
  									<img class="img-fluid" src="  https://semangatMERCUSUAR77.site/assets/images/btn_playnow.png" alt="play-now-in-browser">
  								</div>
  							</a>
  							</div>
  						</div>
...
<script>
        ...
        window.onLogOut = async function(){
            ...
            window.location.href="https://semangatMERCUSUAR77.site/logout";
        ...
        window.json_post(
         ...
         window.sweetAlert= function ( msg , type , title  ,showCancelBtn   ) {
            //check CF error
            var dateNow = "2026-02-13 16:28:51";
