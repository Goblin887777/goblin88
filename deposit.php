...
<script>
    // HANYA FUNGSI UNTUK PANGGIL FORM ISI NOMINAL
    function panggil_isi_nominal($type, $method = 0, $provider = 0, $pID = "") {
        // Tampilkan loading sebentar biar user tau ada proses
        if(typeof showLoadingImgFn === 'function') showLoadingImgFn();

        $.get('load_deposit_form.php', { 
            isgateway: $type, 
            method: $method, 
            provider: $provider, 
            pID: $pID 
        }, function(data) {
            // JURUS ANTI-MIRROR & ANTI-STUCK
            if (data.indexOf('<html') >= 0) { window.location.reload(); return; }

            // MASUKKAN FORM KE CONTAINER (Cek ID container lo, biasanya .content-form atau .methods_form)
            $('.content-form').html(data);
            
            // Matikan loading paksa
            if(typeof removeLoadingImgFn === 'function') removeLoadingImgFn();
            $('.loading-overlay, .preloader').fadeOut();
            
            window.scrollTo(0,0);
            setTimeout(function() { $('#depo_amount, #deposit_amount').focus(); }, 500);
        }).fail(function() {
            if(typeof removeLoadingImgFn === 'function') removeLoadingImgFn();
            sweetAlert("Gagal", "Gagal memuat form isi nominal.", "error");
        });
    }

    $(document).ready(function () {
        // GAK USAH panggil_semua_metode() lagi karena udah ada di HTML lo!

        // LOGIKA KLIK: Langsung dengerin klik dari list bank yang udah ada
        $(document).on('click', '.payment-methods', function(e) {
            e.preventDefault();
            
            // 1. Blokir jika Offline
            if ($(this).hasClass('offline') || $(this).attr('data-form') === "maintenance") {
                sweetAlert("MAINTENANCE", "Metode ini sedang offline, Bos!", "error");
                return false;
            }
            ...
            var pid  = $(this).attr('data-p_id');

            // 2. Panggil form input
            panggil_isi_nominal(type, meth, prov, pid);

            // Efek visual centang
            $('.payment-methods').removeClass('active tick');
            $(this).addClass('active tick');
        });

        // OPTIONAL: Kalau mau otomatis buka QRIS pas halaman load, cukup baris ini:
        // panggil_isi_nominal('flypay', 9); 
    });
</script>
<style>
...
<div class="container-wrapper acc">
  <div class="container container-box noSidePadding">
              ...
              <div class="mdc-tab-scroller__scroll-area mdc-tab-scroller__scroll-area--scroll mdc-tab-scroller__scroll-x" style="margin-bottom: 0px;">
                <div class="mdc-tab-scroller__scroll-content">
                  <ul class="list-inline">
                    <li>
                      <div class="deposit-notice-menu">
                            <!---->
                    <a role="tab" href="deposit.php" data-active="accountdeposit" class="mdc-tab mdc-tab--active" aria-selected="false" tabindex="-1" id="goog_2098347606-FIXED-0">
                  ...
                  <!---->
                    ...
                    <li>
                      <a role="tab" href="withdraw.php" data-active="accountwithdrawal" class="mdc-tab" aria-selected="true" tabindex="0" id="goog_2098347606-FIXED-1">
                        <span class="mdc-tab__content">
                    ...
                    <li>
                      <a role="tab" href="history.php" data-active="accountlastdirecttransfer" class="mdc-tab" aria-selected="false" tabindex="-1" id="goog_2098347606-FIXED-3">
                        <span class="mdc-tab__content">
                    ...
                    <li>
                       <!----><a role="tab" href="history.php" data-active="accounthistory" class="mdc-tab" aria-selected="false" tabindex="-1" id="goog_2098347606-FIXED-2">
                        <span class="mdc-tab__content">
    ...
    <div class="container"><div class="row"><!----></div></div><div class="content">
      <div class="deposit">
    <div class="row">
...
<div class="col-xs-12">
    <div class="acc_safety_info box-wrapper deposit-notice-wraper" style="background: #050B14; border: 1px solid #eeff00; border-radius: 8px;">
        <div class="deposit-notice-content">
            <h4 class="notice__title text-uppercase" style="color: #eeff00;">PEMBERITAHUAN PENTING</h4>
            
            <!-- Area yang dipencet (Trigger) -->
            <div id="wd-toggle" style="cursor: pointer; user-select: none;">
                <p style="margin-bottom: 0; color: #F0F8FF;">
                    <span id="tanda" style="color: #eeff00; font-weight: bold;">[+]</span> 
                    <b>Deposit Wajib Transfer menggunakan kode unik cnth: 50.777.</b>
                </p>

                <!-- Isi yang otomatis terbuka/tertutup -->
                <div id="detail-aturan" style="display: none; margin-top: 10px; border-top: 1px solid #999600; padding-top: 10px;">
                    <p style="color: #F0F8FF;"><b>Aturan Bertransaksi Di <span style="color: #eeff00;">GOBLIN$88</span> :</b></p>
                    <p style="color: #eeff00;"><b>DEPOSIT</b></p>
                    <ul style="padding-left: 20px; color: #F0F8FF;">
                        <li><b>Jika ingin transfer menggunakan nama yang berbeda maka bisa konfirmasi terlebih dahulu ke CS melalui livechat yang tersedia.</b></li>
                        <li><b>Isi formulir deposit sesuai dengan nominal yang di transfer serta tujuan bank yang di transfer.</b></li>
                        <li><b>Untuk rekening tujuan deposit tidak bersifat permanen ( Bisa berubah sewaktu waktu ) jadi harap selalu melakukan pengecekan rekening tujuan deposit terlebih dahulu sebelum ditransfer.</b></li>
                        <li><b>Faktor utama yang membuat rekening sering berubah adalah padat-nya transaksi , hal ini di karena kan slot yang disediakan <span style="color: #eeff00;">Mircusuar 77</span> sangat sering <span style="color: #eeff00;">"GACOR"</span> maka membuat proses transaksi begitu padat.</b></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-xs-12 col-md-8 content-form">
            <div class="methods_form">
                <div class="box-wrapper plr-15" style="flex: 1 0 100%;">
                    <div class="row align-items-center ">
                        <div class="col-md-3 col-xs-12 text-center pay-title">
                            <div>Jasa Pembayaran QRIS <br> (Cek Otomatis)</div>
                        </div>
                        <div class="col-md-9 col-xs-12 d-flex flex-wrap pg_sec" style="align-items: stretch;">
                            <a class="pg_method payment-methods online" href="javascript:void(0);" data-form="flypay" data-method="9">
                                <div class="pg_title online">
                                    <img class="img-fluid provider_logo" alt="qris - flypay" src="https://files.sitestatic.net/sprites/bank_logos/payment_gtway/bank_logos/flypay.png?v=1.63">
                                </div>
                                <div class="pg_bank_logs_sec">
                                    <img class="img-fluid pg_bank_logo" alt="QRIS" src="https://files.sitestatic.net/sprites/bank_logos/payment_gtway/bank_logos/QRIS.png?v=1" style="display:block; margin-left:auto; height:100%; max-height:20px; width:100%; max-width:90px; margin-right:auto">
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <div class="box-wrapper plr-15" style="flex: 1 0 100%;">
                    <div class="row align-items-center">
                        <div class="col-md-3 col-xs-12 text-center pay-title">
                            <div>Konfirmasi Transfer Bank <br> (Manual Cek)</div>
                        </div>
                        <div class="col-md-9 col-xs-12 d-flex flex-wrap">
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="5" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" alt="bank_bca" src="https://files.sitestatic.net/sprites/bank_logos/IDR/bank_bca_col.webp?v=1">
                                </div>
                            </a>
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="5" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" alt="mandiri" src="https://files.sitestatic.net/sprites/bank_logos/IDR/mandiri_col.webp?v=1">
                                </div>
                            </a>
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="5" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" alt="bank_bni" src="https://files.sitestatic.net/sprites/bank_logos/IDR/bank_bni_col.webp?v=1">
                                </div>
                            </a>
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="10" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" alt="bank_bri" src="https://files.sitestatic.net/sprites/bank_logos/IDR/bank_bri_col.webp?v=1">
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <div class="box-wrapper plr-15" style="flex: 1 0 100%;">
                    <div class="row align-items-center">
                        <div class="col-md-3 col-xs-12 text-center pay-title">
                            <div>E-walet <br> (Manual Cek)</div>
                        </div>
                        <div class="col-md-9 col-xs-12 d-flex flex-wrap">
                            <a class="payment-methods payment-methods-items online" data-form="0" data-method="5" href="javascript:void(0);">
    <div class="verti online">
        <img class="img-fluid" src="https://files.sitestatic.net/sprites/bank_logos/ewallet/dana_col.webp?v=2.1">
    </div>
</a>
                            <a class="payment-methods payment-methods-items online" data-form="0" data-method="7" href="javascript:void(0);">
                                <div class="verti online"> 
                                  <img class="img-fluid" src="https://files.sitestatic.net/sprites/bank_logos/ewallet/ovo_col.webp?v=2.1">
                               </div>
                            </a>
                            <a class="payment-methods payment-methods-items online" data-form="0" data-method="6" href="javascript:void(0);">
                                <div class="verti online">
                                    <img class="img-fluid" src="https://files.sitestatic.net/sprites/bank_logos/ewallet/gopay_col.webp?v=2.1">
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <div class="box-wrapper plr-15" style="flex: 1 0 100%;">
                    <div class="row align-items-center">
                        <div class="col-md-3 col-xs-12 text-center pay-title">
                            <div>Pulsa <br> (Manual Cek)</div>
                        </div>
                        <div class="col-md-9 col-xs-12 d-flex flex-wrap">
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="6" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" src="https://files.sitestatic.net/sprites/bank_logos/ewallet/telkomsel_col.webp?v=1">
                                </div>
                            </a>
                            <a class="payment-methods payment-methods-items offline" data-form="maintenance" data-method="6" href="javascript:void(0);">
                                <div class="verti offline">
                                    <img class="img-fluid" src="https://files.sitestatic.net/sprites/bank_logos/ewallet/xl_col.webp?v=1">
                                </div>
                            </a>
                        </div>
                    </div>
            ...
            <div id="nominal-form-container" style="margin-top: 20px; display: none;"></div>
...
<script>
    function load_form($type, $method = 0, $provider = 0, $pID = "") {
        ...
        $.get('load_deposit_form.php', { 
            ...
            if (data.indexOf('<html') >= 0 || data.indexOf('login-form') >= 0) {
                window.location.reload(); 
                return;
            }

            // Masukkan data ke kontainer bawah
            $('#nominal-form-container').html(data).fadeIn();
            
            if(typeof removeLoadingImgFn === 'function') removeLoadingImgFn();
            
            // Scroll ke form nominal
            $('html, body').animate({
                scrollTop: $("#nominal-form-container").offset().top - 50
            }, 500);
            
            setTimeout(function() { 
                $('#depo_amount, #deposit_amount, #deposite_amount').focus(); 
            }, 500);

        }).fail(function(xhr) {
            if(typeof removeLoadingImgFn === 'function') removeLoadingImgFn();
            sweetAlert("Error", "Gagal memuat form.", "error");
        });
    }

    $(document).ready(function() {
        $(document).off('click', '.payment-methods').on('click', '.payment-methods', function(e) {
            e.preventDefault();
            var $this = $(this);

            // Jika Offline, kasih tau user tapi daftar jangan dihilangin
            if ($this.hasClass('offline') || $this.attr('data-form') === "maintenance") {
                sweetAlert("MAINTENANCE", "Metode ini sedang offline, Bos. Silahkan gunakan yang bertanda hijau.", "error");
                return false;
            }
            ...
            load_form(type, meth, prov, pid);
            
            $('.payment-methods').removeClass('active tick');
            $this.addClass('active tick');
        });
        
        // Halaman bersih saat buka pertama kali, gak ada auto-load form.
    });
</script>
<script>
$(document).ready(function() {
    // Fungsi Auto Terbuka/Tertutup kalau dipencet
    $('#wd-toggle').click(function() {
        $('#detail-aturan').slideToggle('fast');
        
        // Ganti tanda [+] jadi [-] biar member tau bisa ditutup lagi
        var tanda = $('#tanda').text();
        $('#tanda').text(tanda == '[+]' ? '[-]' : '[+]');
    });
});
</script>
            ...
            <div class="col-xs-12 col-md-4  fs-sm content-txt">
                <div>
                    <div class="d-none d-md-block fs-sm" i18n="">
                        Goblin$88 are favourite for our speedy crediting of funds to your account
                        and experience. Thus, please use Bank Transfer via your local bank account. We do not accept all
                        kinds of deposit by "Cheque" or "Bank Draft" (Company OR Personal Cheque) as your deposit
                        method.
                        <br> <br>
                        Note: Once you have successfully submitted your deposit form and once your funds is cleared in
                        our account, just leave it to our team to process your transactions as speedy as possible. If
                        more than 10 minutes, let us know by clicking
                        <a class="btn btn-link" href="https://direct.lc.chat/19926541/" target="_blank">here</a> and our
        ...
        <div class=" ">
                ...
                <div class="  footericon-single">
                    <a href="https://www.Goblin88.site/home.php?i=1#"><i class="icon-home"></i><div>HOME</div></a>
                                  ...
                                  <div class="  footericon-single">
                    <a href="promosaya.php"><i class="icon-gift"></i> <div style="text-transform:uppercase;">Promo saya</div></a>
                  </div>
                ...
                <div class=" footericon-single">
                    ...
                    <ul class="list-inline horizontal-hover togglemenu-content" id="fundshover_menu">
                      ...
                      <li>
                        <a href="withdraw.php" (click)="closeNav($event);">
                          <div class="fs-sm mt-1" i18n="">Withdraw</div>
                        </a>
                      </li>
                      <li>
           ...
           <!--region Transfer Wallet Menu -->
            <div class="row no-gutters" id="trans_to_game_menu__game-modal">
                <form action="" method="post" id="tw_transfer_form" class="tw_transfer_form" novalidate="novalidate">
                  <input type="hidden" name="_token" value="8BKmXTvH2YtQtZ4ZDpOYPAW1BrZKdNadim11NQ8g">                    <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $( "#mainContentContainer" ).click(function() {
            $( "#sideNav" ).removeClass( "navContentOpen open" );
            $( "#mainContent" ).removeClass( "navContentOpen sideNavOpen rightSideBarOpen" );
        ...
        // --- TAMBAHAN SKRIP DEPOSIT (QRIS & MANUAL) ---
        $(document).on('click', '.payment-methods', function(e) {
            e.preventDefault();
            
            // 1. Cek kalau ada class 'offline', langsung blokir (Warna Merah)
            if ($(this).hasClass('offline')) {
                sweetAlert("SEDANG DALAM PERBAIKAN", "Metode ini sedang maintenance. Gunakan QRIS atau Bank Manual.", "maintenance");
                return false;
            }
            ...
            // 2. Kirim data ke form deposit (Cuma buat FlyPay & Manual '0')
            if (type === "flypay" || type === "0") {
                load_form(type, meth, prov, pid); // Panggil fungsi load form lo
                
                // Efek visual centang/aktif
                $('.payment-methods').removeClass('active tick');
                $(this).addClass('active tick');
            } else {
                sweetAlert("MAINTENANCE", "Gunakan QRIS untuk sementara.", "warning");
            }
        });

        // --- SKRIP BIAR GAK NUMPUK KE SAMPING (RESPONSIF) ---
        function checkMobile() {
            if ($(window).width() < 768) {
                // Paksa container deposit jadi kolom (ke bawah)
                $('.methods_form').css({
                    'display': 'flex',
        ...
        checkMobile(); // Jalankan pas load
        $(window).resize(checkMobile); // Jalankan pas layar di-resize
    });
</script>
...
<link rel="stylesheet" href="/assets/fancybox/js/jquery.fancybox.min.css">
...
<script>
        ...
        window.json_post(
         ...
         window.sweetAlert= function ( msg , type , title  ,showCancelBtn   ) {
            //check CF error
            var dateNow = "2026-02-21 05:11:16";
...
<script>
$(document).ready(function() {
    // 1. Sinkronisasi User (Tetap Pakai, tapi jangan terlalu sering 1 detik)
    setInterval(function() {
        var userAsli = $('.profile-head .col-xs-7 p').first().text().trim();
        if(userAsli !== "" && userAsli !== "Guest") {
            $('#depo_user').val(userAsli);
        }
    }, 3000); // 3 detik sekali aja biar enteng

    // 2. Fix json_post agar tidak Mirror ke Index
    $('#btnSubmit').off('click').on('click', function(e) {
        ...
        var dataKirim = {
            username: $('#depo_user').val(),
            amount: $('#depo_amount').val(),
            payment_method: $('#input_method').val(),
            // Pastikan token ini beneran ada di meta tag
            _token: $('meta[name="csrf-token"]').attr('content') 
        };

        // Gunakan $.ajax manual kalau json_post bawaan engine lo bermasalah di hosting baru
        $.ajax({
            url: "deposit.php",
            type: "POST",
            data: dataKirim,
            success: function(res) {
                // Jika responnya malah halaman login (Mirroring)
                if(res.indexOf('login') > -1) {
                    window.location.href = 'index.php';
                    return;
                }
                sweetAlert("Berhasil!", "success", "Deposit Terkirim");
                $btn.prop("disabled", false).html("KIRIM");
            },
            error: function(xhr) {
                sweetAlert("Gagal terhubung ke server (Error: "+xhr.status+")", "error", "Error");
                $btn.prop("disabled", false).html("KIRIM");
            }
        });
    });
...
// 3. Script buat ngatasi menu macet (Tanpa ngerusak AJAX)
function fixMenu() {
    $('.modal-backdrop').not('.show').remove(); // Cuma hapus backdrop yang nyangkut
    $('body').css('overflow', 'auto');
}
// Jalankan saat halaman kelar load saja
